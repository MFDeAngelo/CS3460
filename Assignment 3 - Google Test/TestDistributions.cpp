...
void checkBins(const Bins& expected, const Bins& actual)
{
    ...
    {
        ASSERT_EQ(expected[i].minValue, actual[i].minValue) << "Wrong minimum value for bin " << i;
        ASSERT_EQ(expected[i].maxValue, actual[i].maxValue) << "Wrong maximum value for bin " << i;
    }
}
...
void checkTotal(const std::uint32_t expected, const Bins& bins)
{
    ...
    ASSERT_EQ(expected, actual) << "Wrong number of elements across all bins";
}
